...
class JsonRender():
    ...
    def __init__(self,queryset,reverse=False,
                 field_list=[],
                 related_fields=True,
                 query_functions=[],annotates=[],):
    ...
    def render(self):
        ...
        for obj in self.queryset:
                    ...
                    if type(field) == ForeignKey:
                        if field.related_model == User:
                            ...
                            if self.related_fields:
                                ...
                                try: field_value = field_value.pk
                                except AttributeError: field_value = None
            ...
            for func in self.query_functions:
                if func.endswith(')'): 
                    item[func.split('(')[0].replace('.','__')] = eval('obj.'+func)
                else:
                    item[func.replace('.','__')] = eval('obj.'+func+'()')
...
def parse_semantic_date(date,output):
    if output=='date':
